<script lang="ts">
	...
	type Session = SessionsResponse<{ tutor: TutorsResponse<{ teachers: TeachersResponse[] }> }>;
	...
	onMount(async () => {
		const response = await pb.collection('sessions').getList<Session>(0, 50, {
			sort: 'date',
			expand: 'tutor,tutor.teachers',
			filter: ``
		});

		fetchedSessions = response.items;

		const firstdate = dayjs(fetchedSessions[0].date).startOf('day');
		const lastdate = dayjs(fetchedSessions[fetchedSessions.length - 1].date)
			.startOf('day')
			.unix();
		...
		for (let item of fetchedSessions) {
			const date = dayjs(item.date).startOf('day').toISOString();
					...
					{#each day.sessions as session (session.id)}
						...
						>
							<div class="flex items-center gap-3">
								{session.expand?.tutor.name}<TeacherBadge
									name={session.expand?.tutor.expand?.teachers[0].display}
									course={session.expand?.tutor.expand?.teachers[0].course}
								/>
							</div>

							<a href="/book/{session.id}" data-sveltekit-preload-data="tap">Book</a>
						</li>
					{/each}
				</ul>
			</div>
