...
export async function POST({ request }) {
  ...
  if (!pb.authStore.model?.id)
    return json({ error: "you are not signed in" }, { status: 403 });
  ...
  try {
    const record = await admin
      ...
      .getFirstListItem(`email="${pb.authStore.model.email}"`);
		...
		await admin.collection('tutors').create({ user: pb.authStore.model.id, name: pb.authStore.model.name, isNHS: true } satisfies TutorsRecord);
